...
def test_figures_are_svg_only():
    ...
    to forget to regenerate. `README.md` is the one exception: the figures
    carry no title or caption of their own (see `tools/make_figures.py`), so
    their explanation lives there instead, generated by the same script.
    """
    stray = sorted(
        p.name for p in FIGURES.iterdir()
        if p.suffix != ".svg" and p.name != "README.md"
    )
    assert not stray, f"docs/figures/ should hold SVG and README.md only, found: {stray}"
...
def test_the_figures_readme_covers_every_figure():
    """The figures carry no title or caption -- `README.md` is the only place
    their explanation lives, so a figure missing from it is undocumented."""
    readme = (FIGURES / "README.md").read_text()
    for svg in sorted(FIGURES.glob("*.svg")):
        assert svg.name in readme, f"{svg.name} is committed but undocumented"
