...
| document | what it is |
|---|---|
| [`pdf/spar-paper.pdf`](pdf/spar-paper.pdf) | **The paper.** 52 pages: the verdict first (§21.1), the problem stated formally, the four prior methods and how each fails here, what SPAR-PIBT changes and why, the whole system in full detail, and what the measurements said. Source: [`latex/`](latex/). |
...
| [`figures/`](figures/) | The nine result figures, as SVG (renders inline on GitHub) and PDF (what the paper embeds). Start with [`scorecard.svg`](figures/scorecard.svg): one cell per rival per map, answering "is SPAR-PIBT better?" without arithmetic. |
...
`latex/spar.tex` is the preamble and the `\input` list; `latex/sections/`
