...
across four maps this single distinction is worth between $1.9\times$ and
$3.1\times$ throughput and is the largest effect in the system. Two further
...
scoped rather than universal, and we report it that way: AisleFlow's aisle
layer wins on the aisle-shaped maps it was designed for, is far cheaper and far
more productive than either external baseline on every map tested, and
\emph{loses} to plain lifelong PIBT on open grids, where the contention it
manages does not exist. The final sections say exactly where that boundary
falls and why.
\end{abstract}
