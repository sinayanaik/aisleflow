...
% or directly:  latexmk -lualatex -interaction=nonstopmode docs/latex/spar.tex
%
...
\title{\textbf{SPAR}\\[0.2em]
  \large \textnormal{\textbf{S}oft-\textbf{P}riced \textbf{A}isle
  \textbf{R}eversal for lifelong multi-agent pickup and delivery}\\[0.55em]
...
This document describes \textbf{SPAR} --- \emph{Soft-Priced Aisle
Reversal} --- and its planner \textbf{SPAR-PIBT}, and
reports what happened when it was measured. The name is the claim: aisles
\emph{reverse} on demand, and a robot that drives the wrong way down one is
not forbidden but \emph{priced}. Its central design decision is negative and, we
...
scoped rather than universal, and we report it that way, verdict first
(\Cref{sec:results-answer}). Against both published baselines SPAR-PIBT
delivers $13\times$ to $315\times$ their throughput on every map tested, at a
twentieth to a three-hundredth of their cost per timestep, at the strongest
significance five seeds can express; Token Passing delivers nothing at all on
the hardest map. Against plain lifelong PIBT --- the planner SPAR extends, and
the comparison that actually tests the aisle layer --- the answer splits by map
class: ahead by 27\% and 21\% on the two \emph{aisle-constrained} maps, where a
contended aisle has no route around it, and behind by 18\% and 19\% on the two
\emph{open-floor} maps, where cross-aisles provide one. At five seeds the two
losses are significant and the two wins ($p = 0.056$ and $0.063$) are not yet,
though the mechanism's own metrics --- a quarter less driving per delivered
task, half the head-on conflicts --- are significant exactly where throughput is
not. The three mechanisms above are established wherever their mechanism can
act at all. The final sections say exactly where that boundary falls, and what
it would take to settle the part that is still open.
\end{abstract}
...
\texttt{aisleflow} and \texttt{lda\_pibt}; \texttt{SPAR} and
\texttt{SPAR-PIBT} are the method and its planner throughout this document.
\end{center}
