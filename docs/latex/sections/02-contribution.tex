\section{What SPAR-PIBT changes, and why it is different}
...
and what a one-way rule ordinarily means. \Cref{eq:soft} is what SPAR-PIBT
...
\begin{caveat}
The general form of this finding is worth stating separately from SPAR-PIBT,
...
SPAR-PIBT adds a maximum green $T_{\max}$: past $T_{\max}$ steps in one
...
SPAR-PIBT requires corroboration: a group must show signal 1 \emph{and} at least
...
$134$ against $22$ tasks per 1000 timesteps ($p = 0.008$), a six-fold difference
...
SPAR-PIBT, headline config.
  & $+\,O(|A| + |\mathcal{A}|)$ aisle bookkeeping & 0.6--2.0 \\
SPAR-PIBT, all mechanisms
  & same, with entry admission and recovery & 1.4--4.4 \\
...
\bottomrule
\end{tabular}
\caption{Per-timestep cost. Measured columns are means over five seeds; the
spread is across the four maps of \Cref{sec:results}. The headline
configuration is \variant{aisle\_direction\_only}, the one
\Cref{tab:scorecard} reports. The aisle layer costs a small constant factor over PIBT; both
...
are established, the scoping of the aisle layer to aisle-constrained maps is
