\section{What LDA-PIBT changes, and why it is different}
...
and what a one-way rule ordinarily means. \Cref{eq:soft} is what LDA-PIBT
...
Four single-factor comparisons (\code{config.PAIRED\_DESIGNS}), same seeds,
same maps, direction decisions identical, differing only in
\Cref{eq:hard} versus \Cref{eq:soft}: between $1.9\times$ and $3.1\times$
throughput in favour of pricing, significant at $p \leq 0.05$ on every map
where an aisle ever commits a direction. It is the largest single effect
measured anywhere in this system. \Cref{fig:forest} plots it beside every other
mechanism, and \Cref{sec:results-mechanisms} gives the table.

\begin{caveat}
The general form of this finding is worth stating separately from LDA-PIBT,
...
LDA-PIBT adds a maximum green $T_{\max}$: past $T_{\max}$ steps in one
...
switching is significant ($30.5$ against $5.0$ switches per 1000 steps on
\variant{warehouse\_narrow}, $p = 0.024$). The third comparison animation shows
...
LDA-PIBT requires corroboration: a group must show signal 1 \emph{and} at least
...
$0.134$ against $0.022$ tasks per timestep, a six-fold difference obtained
entirely by declining to act on the weakest evidence. The fourth comparison
...
LDA-PIBT (this work)
  & $+\,O(|A| + |\mathcal{A}|)$ aisle bookkeeping & 1.4--4.2 \\
Token Passing \citep{ma2017tp}
  & $O(|A| \cdot \mathrm{A}^\ast(|V| \cdot T_{\mathrm{horizon}}))$ & 34--360 \\
RHCR \citep{li2021rhcr}
  & windowed CBS, exponential in conflicts, capped & 45--85 \\
\bottomrule
\end{tabular}
...
aisle layer wins where aisles are scarce and contended, and loses where they
are not. On \variant{warehouse\_corridors} (five parallel single-file
corridors) aisle direction control is the better configuration; on
\variant{warehouse\_bottleneck} the full method wins on throughput; on the open
\variant{warehouse\_medium} grid, plain lifelong PIBT is clearly better, because
a committed direction costs detours and buys orderly flow through a contention
that is not there. The fifth comparison animation is that losing case, shown at
the same size as the winning ones.

Both facts are consistent with the same explanation, and neither is hidden in
what follows.
