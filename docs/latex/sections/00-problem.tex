...
\variant{warehouse\_bottleneck}, $\lambda = 0.8$ against roughly $0.14$ served.
Between a sixth and a third of offered demand is served; the backlog grows
without bound for the whole run. Three consequences follow, and all three are
routinely misread.
...
roughly $22$ timesteps --- the best figure in the table --- while completing
essentially nothing, because the handful of tasks it finishes are the ones it
finishes in the first few timesteps before the corridor jams forever. Mean
