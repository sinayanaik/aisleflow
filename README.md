# LDA-PIBT — Lifelong Aisle-Managed PIBT

**Directional and congestion-aware PIBT for lifelong multi-agent pickup and delivery.**

A complete, runnable Python implementation of the *Lifelong Aisle-Managed PIBT*
proposal, an extension of
...
```bash
git clone <your-repo-url> lda-pibt
cd lda-pibt
...
| `config.py` | 33, 34 | `Params` + six ablation presets |
...
| `simulator.py` | 30 | the 16-step lifelong loop |
| `experiments.py` | 34, 35 | ablation table and density sweep drivers |
...
| | claim | verdict |
|---|---|---|
| H1 | aisle direction improves throughput in narrow aisles under dense traffic | **partly** — only where the map has a genuine structural bottleneck |
| H2 | hysteresis reduces switching and prevents oscillation | **supported** — `hysteresis_pibt` beats `directional_pibt` on every map |
| H3 | reservations reduce head-on conflicts and deadlocks | **inconclusive** — they also throttle entry enough to cost more than they save |
| H4 | congestion-aware assignment reduces average and tail service time | **supported** — service times drop, though at a throughput cost |
...
| H6 | localized recovery beats a global planner | **supported** — recovery is what keeps the aisle layer alive at all |
...
tests/           96 tests: graph, PIBT, aisle manager, lifelong layer, GUI
experiments/     run_ablation.py, run_density_sweep.py
