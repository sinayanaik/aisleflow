...
<body>
<main>
  <h1>AisleFlow: is it better?</h1>
  <p class="lede">
    Yes on the maps it was designed for, no on open ones, and by a wide margin
    against both published baselines everywhere. This page lets you check that
    claim on any metric, on any map, with the interval and the p-value attached.
  </p>
...
function drawChart() {
  ...
  const entries = [
    ...
    ["var(--series-2)", "AisleFlow variants"],
...
function drawTiles() {
  ...
  // best AisleFlow configuration against plain PIBT, per map family
  const families = [
  ...
  for (const family of families) {
    ...
    host.appendChild(tile(
      `AisleFlow ${family.label}`,
      `${mean >= 0 ? "+" : ""}${mean.toFixed(0)}%`,
      mean >= 0 ? "win" : "loss",
      `Best AisleFlow configuration against plain lifelong PIBT on ` +
      family.maps.map(shortMap).join(" and ") + "."
    ));
  }
  ...
  if (gaps.length) {
    host.appendChild(tile(
      "AisleFlow vs the published baselines",
      "far ahead",
      "win",
...
function provenance() {
  ...
  node.textContent =
    "Generated by tools/dashboard.py from docs/data/ \\u00b7 aisleflow @ " +
